...
### SSE & Streaming Response Monitoring

```javascript
const helpers = require('./lib/helpers');

// Monitor SSE events
const sse = helpers.monitorSSE(page, '/api/stream/', (event, url) => {
  console.log('SSE event:', event);
});

// Capture session data
const session = helpers.captureSessionStream(page, '/api/session/');
// Later: session.sessionId(), session.captured

// Monitor WebSocket messages
const ws = helpers.monitorWebSocket(page, (msg, ws) => {
  console.log('WS:', msg.type, msg.data);
});

// Intercept streaming/chunked responses
const api = helpers.interceptStreamingResponse(page, '/api/agent/', (chunk) => {
  if (chunk.body?.stage) console.log('Stage:', chunk.body.stage);
});
```

See `lib/helpers.js` for full API.
